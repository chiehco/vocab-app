...
it('connects every approved official Unit17 card to its image and reviewed meaning', () => {
  ...
  for (const card of cards) {
    const official = words.find(word => word.wordId === card.officialWordId)!;
    expect(official).toBeDefined();
    expect(getWordBeastAsset(official.wordId, official.word)).toContain(card.illustration!.path);
    expect(getWordDisplaySense(official as WordRecord, []).needsReview).toBe(false);
    expect(approvedUnit17Card(official.word)?.illustration).toEqual(card.illustration);
