...
import { templateGroup, unitItems, allQuestions, validGroup } from '../direct/model';
...
it('submits all 178 Unit 2 items and restores both templates and first answers without SRS changes',async()=>{
 ...
 for(const id of s.questionIds){const q=allQuestions.find(q=>q.questionId===id)!;await updateQuestion(s.id,id,{choice:q.answer});await submitAnswer(s.id,id);await nextQuestion(s.id);}
