...
export default function ResilientBeastImage({ src, word, className = "", alt = "", ...props }: ResilientBeastImageProps) {
  const [failed, setFailed] = useState(false);
  useEffect(() => setFailed(false), [src]);

  if (!failed) return <img {...props} className={className} src={src} alt={alt} onError={() => setFailed(true)} />;
  return (
    <div className={`beast-image-fallback ${className}`} role="img" aria-label={`${word || "單字"} 圖片載入失敗`}>
