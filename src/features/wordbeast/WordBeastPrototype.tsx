...
export default function WordBeastPrototype() {
  ...
  return current && (
    <div className={`wordbeast-page encounter-page ${phase}`}>
      ...
      <main className="encounter-main"><div className="beast-stage"><ExamTierBadge tier={current.tier} /><div className="ink-halo" /><div key={current.record.word} className="beast-visual"><img src={current.image} alt={`${current.record.word} 字獸`} /></div>{phase === "binding" && <><div className="binding-ring" /><div className="true-name">{current.record.word.toUpperCase()}</div><div className="capture-seal">錄</div></>}</div>
        {phase === "encounter" ? <section className="naming-panel"><p className="beast-clue">「{current.record.meaningZh}」</p><p className="naming-instruction">看穿牠的偽裝，喚出英文真名</p><div className="name-choices">{current.choices.map((choice) => <button key={choice} disabled={answering} className={wrongChoice === choice ? "wrong" : ""} onClick={() => answer(choice)}>{choice}</button>)}</div><p className={`encounter-feedback ${wrongChoice ? "visible" : ""}`}>偽名破碎了。再看清牠留下的線索。</p></section>
          : <section className="binding-copy"><p>{current.tier ? `${current.tier} 級字卡・真名已被喚醒` : "真名已被喚醒"}</p><strong>{current.record.word}</strong><span>{current.record.meaningZh}・{current.record.pos}</span><small>{encounterIndex < beasts.length - 1 ? "下一道氣息正在靠近…" : "正在收錄《萬字譜》…"}</small></section>}
      </main>
    </div>
  );
}
