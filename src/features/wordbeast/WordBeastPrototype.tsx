  ...
  if (phase === "archive") {
    return (
      <div className="wordbeast-page archive-page">
        ...
        {selectedBeast && <div className="detail-veil" role="dialog" aria-modal="true"><section className="beast-dossier"><button className="detail-close" onClick={() => setSelectedBeast(null)}>×</button><div className="dossier-portrait"><ExamTierBadge tier={selectedBeast.tier} /><ResilientBeastImage src={selectedBeast.image} word={selectedBeast.record.word} alt={`${selectedBeast.record.word} 字獸`} /><span>封印穩固</span></div><div className="dossier-copy"><p className="wordbeast-eyebrow">真名解讀・{encounterPos(selectedBeast)}</p><h2>{selectedBeast.record.word}</h2><button className="pronounce" onClick={() => speak(selectedBeast.record.word)}>♪　再次喚名</button><p className="dossier-meaning"><small>本題語境義</small>{encounterMeaning(selectedBeast)}</p><p className="dossier-all-meanings"><b>完整釋義</b>{selectedBeast.record.meaningZh || "釋義待補"}</p><div className="dossier-rule" /><dl>{selectedBeast.example && <div><dt>遭遇紀錄</dt><dd>{selectedBeast.example.sentenceEn}</dd><dd className="translation">{selectedBeast.example.sentenceZh}</dd></div>}{selectedBeast.related.length > 0 && <div><dt>同族痕跡</dt><dd className="related-traces">{selectedBeast.related.map((item) => <span className="related-trace" key={item.word}><span><strong>{item.word}</strong><small>{item.relation}</small></span><b>{item.meaning}</b></span>)}</dd></div>}</dl></div></section></div>}
      </div>
    );
  }
