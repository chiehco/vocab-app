...
export default function WordsWorkspace(){
  ...
  return <div className="words-workspace">
    <header className="words-top"><Link to="/" aria-label="回首頁">萬詞譜</Link><h1>單字</h1><Link to="/textbook">課本單元</Link></header>
    ...
    <div className="words-columns">
      ...
      <div className="words-content" ref={panel}>
        ...
        <footer className="words-tools"><Link to={`/units?level=${level==='all'?'LV1':level}&order=${sort==='alpha'?'alphabet':'exam'}`}>每 30 字分組</Link><Link to="/textbook">課本單元</Link><Link to="/review?due=1">今日複習</Link><Link to="/placement">程度測驗</Link></footer>
      </div>
    </div>
  </div>;
}
