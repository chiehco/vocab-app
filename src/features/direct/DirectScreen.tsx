...
import { allQuestions, questions, REVISION, wrongQuestionIds } from './model';
import type { DirectSession } from './model';
import { startSession, updateQuestion, submitAnswer, nextQuestion } from './store';
...
export default function DirectScreen() {
  ...
  return <div className="direct-page">
    ...
    <p className="direct-muted">{s?.questionIds.some(id=>!questions.some(q=>q.questionId===id)) || s?.groupId ? '單元基礎練習：目標字填空與文法搭配。可隨時離開，下次續答。' : '六題新編情境題，尚未校準學測難度。'}點英文查中文，作答後看解析。</p>
    ...
    {!data ? <p>讀取進度中…</p> : !s || s.revision !== REVISION ? <button disabled={busy} className="direct-primary" onClick={() => void act(() => openSession(startSession()))}>開始作答 · 6 題</button> : q ? <>
      ...
      <p className="direct-muted">{s.lookups[q.questionId]?.length ? '本題已查字，會保留提示紀錄。' : '提交前查字會標記為使用提示。'}</p>
      {!q.options.length && <><p>目標義：{q.targetMeaningZh}</p><p className="direct-muted">請填本單元目標詞。系統只比對目標詞，不判定其他同義表達；大小寫不影響結果。</p><label>填入目標詞<input key={`${s.id}:${q.questionId}`} defaultValue={s.choices[q.questionId] ?? ''} disabled={busy || !!a} autoComplete="off" autoCapitalize="none" spellCheck={false} maxLength={120} onChange={e=> {void updateQuestion(s.id,q.questionId,{choice:e.target.value}).catch(()=>setError('未能保存輸入，請重試。'));}} /></label></>}
      ...
      {!a ? <button className="direct-primary" disabled={busy || !s.choices[q.questionId]?.trim()} onClick={() => void act(() => submitAnswer(s.id,q.questionId))}>確認答案</button> : <section className="direct-result">
        <h2>{a.correct ? '答對了' : `${q.options.length ? '本題答案' : '本題目標詞'}：${q.answer}`}</h2><p>{q.targetWord} — {q.targetMeaningZh}</p>
        {!q.options.length && <><p>你的填答：{a.choice}</p><p>{q.sentenceEn}</p><p>{q.sentenceZh}</p></>}
        <p className="direct-muted">{a.hintUsed ? '本次作答前曾查字' : '本次作答前未查字'}</p>
      ...
      {wrong.length > 0 && <button className="direct-primary" disabled={busy} onClick={() => void act(() => openSession(startSession(wrong,s.title,s.groupId,scope)))}>重練錯題（{wrong.length}）</button>}
      <button disabled={busy} onClick={() => void act(() => openSession(startSession(s.questionIds,s.title,s.groupId,scope)))}>再練本輪題目</button>
    </section>}
    <footer><p><Link to="/vocabulary">返回單字字卡</Link></p><p>首次作答：{first.filter(a => a.correct).length} / {first.length} 題正確</p><p className="direct-muted">重練保留首答；本練習不改變正式複習排程。查字使用本機資料，不呼叫翻譯 API。</p>
