import { questionForMode, type DirectAttempt, type PracticeMode } from './model';
...
export function orderPracticeScope(scope: string[], attempts: DirectAttempt[], mode: PracticeMode, random = Math.random) {
  const ids = [...new Set(scope.map(id => questionForMode(id, mode)))];
  const counts = new Map<string, number>();
  for (const a of attempts) counts.set(a.questionId, (counts.get(a.questionId) ?? 0) + 1);
